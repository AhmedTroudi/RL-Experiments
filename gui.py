...
class RobotGUI:
    def __init__(
        ...
        rows, cols = self.map_data.shape[0], self.map_data.shape[1]
    ...
    def draw_map(self):
        ...
        for row in range(self.map_data.shape[0]):
            for col in range(self.map_data.shape[1]):
                ...
                if cell_value == 0:
                    ...
                    self.canvas.create_rectangle(
                        x1, y1, x2, y2, fill="brown", outline="black"
                    )  # Obstacle
    ...
    def move_robot(self, new_position: Tuple):
        ...
        x2, y2 = x1 + self.cell_size, y1 + self.cell_size
        if (
            new_position != self.goal_position
        ):  # color in blue cells where the robot steps more than once
        ...
        if (
            new_position not in self.trail_positions
            and new_position != self.goal_position
